...
Based on the analysis, I have concluded that, 

1. manual transmission on average performs `r round(7.245, 2)` miles per gallon better than automatic transmission. However, transmission type (am) alone explains only 33.85% of the variance.

2. After adding other variables, manual transmission alone improves mgp by `r round(2.9358, 2)` compared to automatic transmission while holding weight and qsec (1/4 mile time) constant. These three variables (am, wt, qsec) collectively explain 83.36% variance of the outcome.

# About the assignmenet
...
* **null hypthesis:** mpg of manual and automatic cars are no different.
* **alternative hypothesis:** there is significant difference between mpg manual vs automatic cars.
...
We can also check for average mileage (mpg) for automatic and manual cars.
```{r}
autoMeanMPG <- mean(mtcars$mpg[mtcars$am == 0])

manualMeanMPG <- mean(mtcars$mpg[mtcars$am == 1])
```
Average mileage for automatic cars is: `r, autoMeanMPG`.
Average mileage for automatic cars is: `r, manualMeanMPG`
...
The adjusted R squared is 0.3385 which means our model only explains 33.85% variance.
...
summary(fullModel)
```
As expected, this model has R-squared of 0.869. However, none of the cofficients are significant at 0.05 level.
...
summary(stepModel)
```
This model is far more efficient in terms of variables (wt, qsec, am) which have significance in explaining the variance in depdendent variables. The model explains 0.85 of the variance.
From the coefficients of this model we can conclude that, holding wt and qsec constant, manual cars on average get 2.9 extra miles per gallon than automatic.

```{r}
#confint(stepModel)["am",]
```
